...
def getnextevent():
    ...
    if (
        lowest_timed[0]
        - tz.localize(datetime.datetime.strptime(lowest_allday[0], "%Y-%m-%d"))
    ).seconds < 86400:
        start_time_print = (
            humanize.naturalday(lowest_timed[0]).title()
            + "-"
            + datetime.datetime.strftime(lowest_timed[0], "%H:%M")
        )
    ...
    ) == str(datetime.datetime.strptime(lowest_allday[0], "%Y-%m-%d")):
